...
def read_float(v):
    ...
    f = 0
    try:
        f = float( v[0]+v[1:].replace(' ', '').replace('+', 'e+').replace('-', 'e-') )
    except ValueError:
        f = ''
    return f
...
def read_table(lines):
    ...
    https://t2.lanl.gov/nis/parse/intro07.html
    https://t2.lanl.gov/nis/parse/intro08.html
    https://t2.lanl.gov/nis/parse/intro09.html
    """
